...
async def get_question(conn, question_id):
    ...
    result = await conn.execute(
        choice.select().where(choice.c.question_id == question_id)
...
async def vote(conn, question_id, choice_id):
    ...
    if not record:
        msg = "Question with id: {} or choice id: {} does not exist"
        raise RecordNotFound(msg.format(question_id, choice_id))


async def add(conn, question_text, choices):
    async with conn.begin():
        insert_question = (question.insert()
             .values([
                 {"question_text": question_text, "pub_date": "2015-12-15 17:17:49.629+02"},
             ])
             .returning(question.c.id))
        result = await conn.execute(insert_question)
        data = await result.fetchone()
        print(data)
        insert_choices = (choice.insert()
            .values([
                {"choice_text": choices[i], "question_id": data[0]} for i in range(3)]))
        await conn.execute(insert_choices)
